...
describe('runEmbed CLI flag wiring (--stale --source)', () => {
  test('--slugs-file reads newline slugs and preserves --source threading', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'gbrain-slugs-file-'));
    const file = join(dir, 'slugs.txt');
    writeFileSync(file, 'alpha\n\n beta \n', 'utf8');
    const seen: Array<{ slug: string; opts: unknown }> = [];
    const engine = mockEngine({
      getPage: async (slug: string, opts: unknown) => {
        seen.push({ slug, opts });
        return { slug, title: slug, compiled_truth: 'truth', timeline: '' };
      },
      getChunks: async () => [
        { chunk_index: 0, chunk_text: 'truth', chunk_source: 'compiled_truth', embedded_at: null, token_count: 1 },
      ],
      upsertChunks: async () => {},
    });

    try {
      await runEmbed(engine, ['--slugs-file', file, '--source', 'media-corpus', '--dry-run']);
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }

    expect(seen).toEqual([
      { slug: 'alpha', opts: { sourceId: 'media-corpus' } },
      { slug: 'beta', opts: { sourceId: 'media-corpus' } },
    ]);
  });
