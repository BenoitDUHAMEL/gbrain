...
export async function runEmbed(engine: BrainEngine, args: string[]): Promise<EmbedResult | undefined> {
  ...
  if (slugsFileIdx >= 0) {
    ...
    if (!filePath || filePath.startsWith('--')) {
      console.error('Usage: gbrain embed --slugs-file <path> [--source <id>] [--dry-run]');
      process.exit(1);
    }
    ...
    if (!slug) {
      console.error('Usage: gbrain embed [<slug>|--all|--stale|--slugs s1 s2 ...|--slugs-file path] [--source <id>] [--dry-run]');
      process.exit(1);
    }
...
async function embedPage(
  ...
  // Embed chunks without embeddings; skip blank chunks because some local
  // providers return zero-length vectors for empty input.
